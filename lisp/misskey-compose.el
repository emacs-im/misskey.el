...
(defun misskey-compose--status-fields ()
  "Return meaningful current draft state, without idle command hints."
  (let* ((items (appkit-chat-compose-items))
         (files (cl-loop for item in items sum (length (plist-get item :attachments))))
         (status (appkit-compose-status-text)))
    (delq nil
          (list
           (when (> files 0) (list :label "Files" :value (number-to-string files)))
           (when (> (length items) 1)
             (list :label "Notes" :value (number-to-string (length items))))
           (when misskey-compose--recipient-status
             (list :label "Recipient lookup" :value misskey-compose--recipient-status))
           ...
           (list :label "Visibility"
                 :value (alist-get misskey-compose-visibility misskey-compose--visibility-choices))
           (list :label "Characters"
                 :value (mapconcat
                         (lambda (item)
                           (let ((count (length (or (plist-get item :text) ""))))
                             (format "%d/%s%s" count
                                     (or misskey-compose--max-note-text-length "?")
                                     (if (and misskey-compose--max-note-text-length
                                              (> count misskey-compose--max-note-text-length))
                                         " OVER" ""))))
                         items " | "))
           (unless (equal misskey-compose--meta-status "Ready")
             (list :label "Limit" :value misskey-compose--meta-status))
           (when misskey-compose-cw (list :label "CW" :value misskey-compose-cw))
           (when misskey-compose-local-only (list :label "Federation" :value "Local only"))
           (when status (list :label "State" :value status))))))
...
(cl-defun misskey-compose-open
    (&optional account &key reply-id renote-id target-label
               target-note (visibility 'public) (cw nil cw-p)
               local-only recipients)
...
TARGET-NOTE is required for replies/quotes and supplies the original
payload for privacy-safe audience, CW, local-only and specified-
recipient defaults. TARGET-LABEL describes it. VISIBILITY is public,
home, followers or specified. CW, LOCAL-ONLY and RECIPIENTS (ID . label
pairs) apply to every part. Instance limits are loaded asynchronously;
publishing is blocked until a valid limit arrives."
  ...
  (when (and (or reply-id renote-id) (null target-note))
    (user-error
     "Original note metadata is required to compose a safe reply or quote"))
  ...
  (let*
      ((app (misskey-app account))
       ...
       (author
        (and target-note
             (misskey-user-id (misskey-note-user target-note))))
       (audience
        (and target-note
             (let ((value (alist-get 'visibility target-note)))
               (and (stringp value) (intern value)))))
       (target-local
        (and target-note (eq (alist-get 'localOnly target-note) t))))
    (when target-note
      (unless
          (and
           (equal (misskey-note-id target-note)
                  (or reply-id renote-id))
           (assq audience misskey-compose--visibility-choices))
        (user-error "Invalid compose target metadata"))
      (when
          (and renote-id
               (or (eq audience 'specified)
                   (and (eq audience 'followers)
                        (not (equal author self)))
                   (alist-get 'channelId target-note)))
        (user-error
         "Cannot quote specified, another user's followers-only, or channel notes"))
      (unless
          (misskey-compose--audience-allowed-p visibility audience)
        (setq visibility audience))
      (setq local-only (or local-only target-local))
      (unless cw-p
        (setq cw
              (let ((warning (alist-get 'cw target-note)))
                ...
                (mapcar
                 (lambda (id)
                   (cons id
                         (if (equal id author)
                             (misskey-user-label
                              (misskey-note-user target-note))
                           id)))
                 (delete-dups
                  (cl-remove self (append ids (list author)) :test
                             #'equal)))))))
    (let
        ((buffer
          (generate-new-buffer
      ...
      (pop-to-buffer buffer) (misskey-compose-mode)
      ...
      (appkit-chat-compose-setup :app app :context-function
                                 #'misskey-compose--context
                                 :status-fields-function
                                 #'misskey-compose--status-fields
                                 :parts-function
                                 #'misskey-compose--parts)
      (misskey-compose-refresh-metadata) buffer)))
...
(defun misskey-compose-set-visibility (&optional visibility)
  ...
  (let*
      ((choices
        (cl-remove-if-not
         (lambda (entry)
           (misskey-compose--audience-allowed-p (car entry)))
         misskey-compose--visibility-choices))
       (label
        (and (null visibility)
             (completing-read "Visibility: " (mapcar #'cdr choices)
                              nil t nil nil
...
(defun misskey-compose-set-cw (warning)
  ...
  (interactive
   (list
    (read-string "Content warning (empty clears): " misskey-compose-cw)))
  (when (appkit-compose-operation-active-p)
    (user-error "Wait for the current publish request to finish"))
  (unless (and (stringp warning) (<= (length warning) 100))
    (user-error
     "A content warning accepts at most 100 Unicode characters"))
  (let ((value (unless (string-empty-p warning) warning)))
    (unless (equal value misskey-compose-cw)
      (setq misskey-compose-cw value) (appkit-compose-touch))))
...
(defun misskey-compose-add-recipient (handle)
  ...
  (let
    ...
    (setq misskey-compose--recipient-status
          (format "Resolving %s" handle))
    (force-mode-line-update)
    (misskey-compose--read 'recipient "users/show" parameters
                           (lambda (user)
                             (let
                                 ((id
                                   (and (listp user)
                                        (misskey-user-id user))))
                               (cond
                                ((not (misskey--valid-user-id-p id))
                                 (setq
                                  misskey-compose--recipient-status
                                  "Invalid user response; C-c C-r retry"))
                                ((equal id
                                        (misskey--account-remote-user-id
                                         (misskey-compose--account)))
                                 (setq
                                  misskey-compose--recipient-status
                                  nil)
                                 (message
                                  "You are already included as sender"))
                                (t
                                 (setq
                                  misskey-compose--recipient-status
                                  nil)
                           ...
                           (lambda (failure)
                             (setq misskey-compose--recipient-status
                                   (format "%s; C-c C-r retry" failure))
                             (force-mode-line-update)))))
...
(defun misskey-compose-remove-recipient (id)
  ...
  (setq misskey-compose--recipient-status nil
        misskey-compose-recipients
        (assoc-delete-all id misskey-compose-recipients))
  (appkit-compose-touch))
...
(defun misskey-compose-refresh-metadata ()
  ...
  (force-mode-line-update)
  (misskey-compose--read 'meta "meta" '(:detail :json-false)
                         (lambda (payload)
                           (let
                               ((limit
                                 (and (listp payload)
                                      (alist-get 'maxNoteTextLength
                                                 payload))))
                             (if (and (integerp limit) (> limit 0))
                                 (setq
                                  misskey-compose--max-note-text-length
                                  limit misskey-compose--meta-status
                                  "Ready")
                         ...
                         (lambda (failure)
                           (setq misskey-compose--meta-status
                                 (format "%s; C-c C-m retry" failure))
                           (force-mode-line-update))))

;;; Publication snapshot

(cl-defun misskey-compose--audience-allowed-p
    (visibility &optional (ceiling misskey-compose--target-visibility))
  "Whether VISIBILITY is at least as restrictive as CEILING."
  (and (assq visibility misskey-compose--visibility-choices)
       (or (null ceiling)
           (assq visibility (memq (assq ceiling misskey-compose--visibility-choices)
                                  misskey-compose--visibility-choices)))))
...
(defun misskey-compose-send ()
  ...
  (let*
    ...
    (appkit-compose-operation-begin 'publish :label label
                                    :cancel-function
                                    (lambda ()
                                      (misskey-compose--abort-submission
                                       buffer token)))
    (setq-local buffer-read-only t) (message "%s" label)
    (condition-case err
        (misskey-compose--send-next buffer account
                                    (list :items items :index 0
                                          ...
                                          :cw
                                          (and misskey-compose-cw
                                               (copy-sequence
                                                misskey-compose-cw))
                                          :local-only
                                          misskey-compose-local-only
                                          :visible-user-ids
                                          (vconcat
                                           (mapcar
                                            (lambda (entry)
...
(defun misskey-compose--start-request (buffer token start callback &optional progress)
  "Start one request belonging to BUFFER's publication TOKEN.
START receives success, failure, and optional progress callbacks.  All
callbacks share one lifetime; synchronous completion cannot overwrite the
next request's cancellation handle."
    ...
    (cl-flet ((current-p ()
      ...
      (cl-flet ((settle ()
        ...
        (setq request
              (funcall start
                       (lambda (payload)
                         (when (settle) (funcall callback payload)))
                       (lambda (failure)
                         (when (settle)
                           (misskey-compose--handle-error buffer token failure)))
                       (when progress
                         (lambda (event)
                           (when (current-p) (funcall progress event)))))))
      (when (and request (current-p))
        (with-current-buffer buffer (setq-local misskey-compose--request request))))
    request))
...
(defun misskey-compose--abort-submission (buffer token)
  "Cancel BUFFER's request and retire TOKEN, even after Surface revocation."
  (when (buffer-live-p buffer)
    (with-current-buffer buffer
  ...
  (let* ((index (plist-get submission :index))
    ...
    (with-current-buffer buffer
      (appkit-compose-operation-update
       (appkit-compose-operation-owner)
       :label (format "Publishing note %d/%d..." (1+ index) (length (plist-get submission :items)))
       :progress nil))
    (misskey-compose--start-request
     buffer token
     (lambda (callback errback _progress)
       (misskey-http-post "notes/create" (misskey-compose--note-parameters submission item)
     ...
     (lambda (payload)
       (if-let* ((note-id (misskey-compose--created-note-id payload)))
           (let* ((next-index (1+ index))
                  (remaining (cl-subseq (plist-get submission :items) next-index)))
             (if remaining
                 (let ((next (copy-sequence submission)))
                   (misskey-compose--persist-confirmed buffer token remaining note-id)
         ...
         (misskey-compose--handle-error
          buffer token "Misskey returned success without a nonempty created note ID; the remote outcome is unknown"))))))

(defun misskey-compose--upload-attachment (buffer account submission note-index attachment-index)
...
BUFFER's NOTE-INDEX selects the draft entry."
  (let* ((item (nth note-index (plist-get submission :items)))
    ...
    (with-current-buffer buffer
      (appkit-compose-operation-update
       (appkit-compose-operation-owner)
       :label (if (> count 1) (format "Uploading %s %d/%d" name index count)
                (format "Uploading %s..." name))
       :progress nil))
    (misskey-compose--start-request
     buffer token
     (lambda (callback errback progress)
       (misskey-http-upload-file path callback :errback errback :account account
                                 :owner (plist-get submission :owner) :progress progress))
     (lambda (payload)
       (if-let* ((drive-id (misskey-compose--drive-file-id payload)))
           (let ((items (misskey-compose--install-drive-id submission note-index attachment-index drive-id)))
             (misskey-compose--persist-items buffer token (cl-subseq items note-index))
             (misskey-compose--send-next buffer account submission))
         (misskey-compose--handle-error
          buffer token "Misskey returned success without a nonempty uploaded file ID; the remote outcome is unknown")))
     (lambda (event) (misskey-compose--upload-progress buffer name index count event)))))

(defun misskey-compose--upload-progress (buffer name index count event)
  "Present a current upload EVENT for NAME at INDEX of COUNT in BUFFER."
  (with-current-buffer buffer
    (let ((progress (plist-get event :progress)))
      (appkit-compose-operation-update
       (appkit-compose-operation-owner)
       :label (cond
               ((and (numberp progress) (>= progress 1.0)) (format "Processing %s..." name))
               ((> count 1) (format "Uploading %s %d/%d" name index count))
