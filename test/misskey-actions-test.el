  ...
  (misskey-test-with-session
    (let*
        ((misskey--apps (make-hash-table :test #'equal))
         ...
         (note (misskey-actions-test--note)) first second requests)
      (unwind-protect
                ...
                (((symbol-function 'message) #'ignore)
                 ...
                 ((symbol-function 'misskey-http-read)
                  (lambda
                    ...
                    'read-request)))
              (setq first (misskey-search "one" account) second
                    (misskey-search "two" account)))
            (misskey-test-drain first)
            (cl-letf
                (((symbol-function 'message) #'ignore)
                 ((symbol-function 'misskey-app)
                  (lambda (&optional _account) app))
              ...
              (misskey-actions-perform 'react note :value ":wave:"
                                       :account account)
              (misskey-test-drain first)
              (dolist (view (list first second))
                (with-current-buffer (appkit-surface-buffer view)
                  ...
                  (should
                   (string-match-p "your reaction :wave:"
                                   (buffer-string)))))
              (misskey-actions-perform 'delete-note note :account
                                       account)
              (misskey-test-drain first)
              (should-not (misskey-test-visible-note-keys first))
              (should-not (misskey-test-visible-note-keys second)))
            (should (= (length requests) 3)))
        (when (appkit-surface-p first)
          (kill-buffer (appkit-surface-buffer first)))
        (when (appkit-surface-p second)
          (kill-buffer (appkit-surface-buffer second)))
...
(defun misskey-actions-test--app ()
  ...
  (let
    ...
    (appkit-app-start misskey--app-type :identity
                      (list 'actions-test (make-symbol "app")) :input
